...
def execute_conflict(graph, conflict_feature):
    """
    This function adds vertices and edges depending on the feature conflict.
    Effectively, the conflict information get executed. This method can skip
    feature conflicts which may reference ONLY isoforms. Skipped conflicts are printed.
    ...
    aa_after = conflict_feature.location.end + 0
    ...
    vertices_before, vertices_after = _get_all_vertices_before_after(
        graph, aa_before, aa_after, conflict_feature.ref
    )
    if len(vertices_before) == 0 or len(vertices_after) == 0:
        # Check if we have vertices, if not simply skip
        print("No Vertices retrieved for protein {}, using CONFLICT: {} (referencing: {}). Skipping...".format(
            graph.vs[0]["accession"], conflict_feature.id, conflict_feature.ref))
        return

    # Now we check if we skip or add nodes
    text = conflict_feature.qualifiers["note"]
    ...
    if text.lower().startswith("missing"):
        ...
        for aa_in_list, aa_out_list in zip(vertices_before, vertices_after):
            ...
            for aa_in in aa_in_list:
                for aa_edge_in in list(graph.es.select(_target=aa_in)):  # Get all incoming edges
                    edge_list.append(
                        ((aa_edge_in.source, first_node), [*_get_qualifiers(aa_edge_in), conflict_feature],)
                    )
            for aa_out in aa_out_list:
                for aa_edge_out in list(graph.es.select(_source=aa_out)):  # Get all outgoing edges
                    edge_list.append(
...
def _get_all_vertices_before_after(graph, aa_before: int, aa_after: int, reference: str):
    """
    Get the vertices which are at the beginning and end of the referencing conflict.
    ...
    if reference is None:
        ...
        # since this conflict references explicitly a isoform!
        ...
        # this case happens e.g. if the user wants conflicts but no isoforms,
        # but since there are conflicts specifically referencing isoforms, we return
        # nothing

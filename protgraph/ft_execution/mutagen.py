...
def execute_mutagen(graph, mutagen_feature):
    """
    This function adds vertices and edges depending on the feature mutagen.
    Effectively, the mutagen information get executed. This method can skip
    feature mutagens which may reference ONLY isoforms. Skipped mutagens are printed.

    NOTE: This transforms the graph without returning it!

    Following Keys are set here:
    Nodes: <None>
    Edges: "qualifiers" ( -> adds Mutagen)
    """
    # Get start and end position first
    # NOTE: Shifted by 1 due to the __start__ node beeing at 0
    aa_before = mutagen_feature.location.start + 1
    aa_after = mutagen_feature.location.end + 0
    ...
    vertices_before, vertices_after = _get_all_vertices_before_after(
        graph, aa_before, aa_after, mutagen_feature.ref
    )
    if len(vertices_before) == 0 or len(vertices_after) == 0:
        # Check if we have vertices, if not simply skip
        print("No Vertices retrieved for protein {}, using MUTAGEN: {} (referencing: {}). Skipping...".format(
            graph.vs[0]["accession"], mutagen_feature.id, mutagen_feature.ref))
        return

    # Now we check if we skip or add nodes
    text = mutagen_feature.qualifiers["note"]
    ...
    if text.lower().startswith("missing"):
                ...
                for aa_edge_in in list(graph.es.select(_target=aa_in)):  # Get all incoming edges
                    for aa_out in aa_out_list:
                        for aa_edge_out in list(graph.es.select(_source=aa_out)):  # Get all outgoing edges
                            ...
                            # for that edge (At least the MUTAGEN feature)
                            edge_list.append(
                                (
                                    (aa_edge_in.source, aa_edge_out.target),
                                    [*_get_qualifiers(aa_edge_in), mutagen_feature],
                                )
                            )
        ...
        # MUTAGEN can reference multiple substitutions
        for y in y_s.split(","):
            y = y.strip().replace(" ", "")
            # TODO is such a combination enough?
            for aa_in_list, aa_out_list in zip(vertices_before, vertices_after):
                ...
                for aa_in in aa_in_list:
                    for aa_edge_in in list(graph.es.select(_target=aa_in)):  # Get all incoming edges
                        edge_list.append(
                            ((aa_edge_in.source, first_node), [*_get_qualifiers(aa_edge_in), mutagen_feature],)
                        )
                for aa_out in aa_out_list:
                    for aa_edge_out in list(graph.es.select(_source=aa_out)):  # Get all outgoing edges
                        edge_list.append(

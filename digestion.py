...
def _digest_via_trypsin(graph):
    ''' Digestion via Trypsin
        ...
        1: One edge from K or R to __end__ and
        2: One edge from __start__ to the next nodes target
    ...
    k_s_edges_remaining = [y for x in k_s_edges for y in x if graph.vs[y.target]["aminoacid"] != "P"]
    r_s_edges_remaining = [y for x in r_s_edges for y in x if graph.vs[y.target]["aminoacid"] != "P"]
    ...
    # Explicitly we add ->
    trypsin_in = [(__start_node__, k.target) for k in k_s_edges_remaining + r_s_edges_remaining] # edges for Nodes which should have an edge to __start__
    trypsin_out = [(k.source, __end_node__) for k in k_s_edges_remaining + r_s_edges_remaining] # edges for Nodes which should have an edge to __end__
